            ...
            if self.tracing_results.is_none() {
                ...
                if cfg!(target_os = "macos") {
                    if let Some(samply_outfile) = tracing_config.samply_outfile.as_ref() {
                        ...
                        std::fs::copy(
                            tracing_data.outfile.path().with_extension("syms.json"),
                            samply_outfile.with_extension("syms.json"),
                        )?;
                    }
                ...
                let fold_data = if cfg!(target_os = "macos") {
                    ...
                    for lib in &loaded.libs {
                        if let Ok(map) = symbol_manager
                            ...
                            .await
                        {
                            symbol_maps.push(Some(map));
                        } else {
                            ProgressTracker::eprintln(format!(
                                "Failed to load symbol map for {}",
                                lib.path
                            ));
                            symbol_maps.push(None);
                        }
                    }
